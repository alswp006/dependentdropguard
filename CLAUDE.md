...
- ⚠️ 문구가 모호해 사용자가 무엇을 하는지 이해 못 함 반복 → 화면 패킷 프롬프트에 '레이블·버튼 문구는 결과를 말할 것("확인"이 아니라 "3만원 저축하기") — 플레이스홀더 문구 금지' 경고 추가 (신뢰도 95%, 88회 관찰)
- ⚠️ 빠져나갈 길이 없는 화면(막다른 길) 반복 → 화면 패킷 프롬프트에 '모든 화면에 돌아가거나 진행할 경로를 둘 것 — 뒤로가기·홈·다음 중 최소 하나' 경고 추가 (신뢰도 95%, 286회 관찰)
...
- [ui] 앱의 모든 화면 경로를 단일 라우트 상수 모듈에 정의하고 동적 파라미터 경로까지 포함해 라우터에 먼저 전부 등록한 뒤, 아직 구현되지 않은 화면은 default export를 가진 플레이스홀더 모듈로 채워 라우터가 항상 컴파일·빌드를 통과하게 하라. (60% · 타 앱 1회 — 맹신 금지)
